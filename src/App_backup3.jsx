...
/**
 * AutoLabel Pro v7.2 — Grouped & Filterable Edition
 *
 * New in v7.2:
 * - Grouped labeled objects by category with collapsible accordions
 * - Filter chips to show/hide categories
 * - Canvas dimming for non-matching boxes when filter active
 *
 * Previous UX Improvements:
 * - Import button in left sidebar (file management zone)
 * - Annotation mode consolidated to canvas toolbar only
 * - Clear button with confirmation dialog
 * - Detection overwrite warning modal
...
const exportToCOCO = (images, regions) => {
  ...
  const cocoImages = images.map((img, idx) => {
    (regions[img.id] || []).forEach((r) => {
      ...
      annotations.push({
        id: annotationId++, image_id: idx + 1, category_id: categoryMap[r.label],
        bbox: [Math.round(r.x), Math.round(r.y), Math.round(r.w), Math.round(r.h)],
        area: Math.round(r.w * r.h), iscrowd: 0, score: r.confidence,
      });
    });
        ...
        <form onSubmit={handleSubmit}>
          ...
          {filteredSuggestions.length > 0 && (
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: "#718096", marginBottom: 6 }}>Existing classes:</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                {filteredSuggestions.slice(0, 8).map((s) => (<button key={s} type="button" onClick={() => setLabel(s)} style={{ padding: "4px 10px", fontSize: 11, background: "#F7FAFC", border: "1px solid #E2E8F0", borderRadius: 4, cursor: "pointer" }}>{s}</button>))}
              </div>
            </div>
          )}
          <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
...
const FilterChip = ({ label, count, color, isActive, onClick }) => (
  <button
    ...
    style={{
      ...
      fontSize: 11,
      fontWeight: 500,
      background: isActive ? (color ? `${color}20` : PRIMARY_LIGHT) : "#F7FAFC",
      color: isActive ? (color || PRIMARY_COLOR) : "#718096",
      border: `1px solid ${isActive ? (color || PRIMARY_COLOR) : "#E2E8F0"}`,
      borderRadius: 16,
      cursor: "pointer",
...
const CategoryAccordion = ({ label, color, regions, isExpanded, onToggle, selectedRegions, onSelectRegion, onDoubleClick, hoveredRegion, setHoveredRegion, activeFilter }) => {
        ...
        <div style={{ border: "1px solid #E2E8F0", borderTop: "none", borderRadius: "0 0 6px 6px", background: "#FFF" }}>
          {regions.map((region) => {
            ...
            return (
              <div
                key={region.id}
                onClick={(e) => onSelectRegion(region.id, e)}
              ...
              >
                <span style={{ flex: 1, fontSize: 11, color: "#4A5568" }}>
                  {region.label}
                </span>
                <span style={{ fontSize: 10, color: "#A0AEC0" }}>
                  {(region.confidence * 100).toFixed(0)}%
                </span>
                <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 8, background: `${status.color}15`, color: status.color }}>
...
export default function AutoLabelProV7() {
  ...
  // Filter state (NEW)
  const [activeFilter, setActiveFilter] = useState(null); // null = "All", or label string
  ...
  // Group regions by label (NEW)
  const groupedRegions = useMemo(() => {
    const groups = {};
    currentRegions.forEach((region) => {
      if (!groups[region.label]) {
        groups[region.label] = [];
      }
  ...
  const handleUndo = useCallback(() => {
    if (historyIndex > 0) {
      ...
      setRegions(JSON.parse(history[historyIndex - 1]));
      showToast("Undo", "info");
    }
  ...
  // Auto-expand first category when regions change
  useEffect(() => {
  ...
  const applySensitivityPreset = (key) => {
    ...
    if (currentModelConfig?.thresholds) {
      ...
      Object.entries(currentModelConfig.thresholds).forEach(([k, v]) => {
        newT[k] = Math.max(v.min, Math.min(v.max, v.default * preset.multiplier));
      });
      setThresholds(newT);
    }
  };
  ...
  const handleSelectRegion = (id, e) => {
    ...
    if (e?.metaKey || e?.ctrlKey) setSelectedRegions((prev) => prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]);
  ...
  const handleCanvasMouseDown = (e) => {
    ...
    if (annotationMode === "draw") {
      setIsDrawing(true);
      setDrawStart(canvasCoords);
      setDrawCurrent(canvasCoords);
      setSelectedRegions([]);
  ...
  const handleCanvasMouseUp = () => {
    if (isDrawing && drawStart && drawCurrent && currentImage) {
      ...
      if (w > 10 && h > 10) {
        ...
        setRegions(newRegions);
        saveToHistory(newRegions);
        setSelectedRegions([newRegion.id]);
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key === "Escape") { setSelectedRegions([]); setIsDrawing(false); setIsDragging(false); setIsResizing(false); setActiveFilter(null); }
  ...
  // Check if region should be dimmed (not matching active filter)
  const isRegionDimmed = (region) => {
    if (!activeFilter) return false;
    return region.label !== activeFilter;
  };
  ...
  return (
    <div style={s.container}>
      ...
      <ConfirmModal isOpen={showClearConfirm} onClose={() => setShowClearConfirm(false)} onConfirm={executeClearCurrentDetections} title="Clear All Annotations?" message={`This will remove all ${currentRegions.length} annotations from this image. This action can be undone with Ctrl+Z.`} confirmText="Clear All" confirmColor="#E53E3E" />
      <ConfirmModal isOpen={showDetectOverwriteConfirm} onClose={() => { setShowDetectOverwriteConfirm(false); setPendingDetectAction(null); }} onConfirm={handleConfirmDetect} title="Overwrite Existing Annotations?" message={pendingDetectAction === 'current' ? `This image has ${currentRegions.length} existing annotation(s). Running detection will replace them. Continue?` : `There are ${totalRegionsAllImages} existing annotations. Running detection will replace them. Continue?`} confirmText="Replace & Detect" confirmColor={PRIMARY_COLOR} />
      ...
      <header style={s.header}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => initiateDetection('current')} disabled={!currentImage || modelStatus !== "connected" || isDetecting} style={s.btn(true, !currentImage || modelStatus !== "connected" || isDetecting)}>{isDetecting ? "Detecting..." : "▶ Detect Current"}</button>
          <button onClick={() => initiateDetection('all')} disabled={images.length === 0 || modelStatus !== "connected" || isDetecting} style={{ ...s.btn(false, images.length === 0 || modelStatus !== "connected" || isDetecting), background: images.length === 0 || modelStatus !== "connected" || isDetecting ? "#A0AEC0" : PRIMARY_LIGHT, color: images.length === 0 || modelStatus !== "connected" || isDetecting ? "#FFF" : "#2B6CB0" }}>{isDetecting ? "Processing..." : `▶▶ All (${images.length})`}</button>
        </div>
      </header>
      ...
      <div style={s.controls}>
        <div style={{ minWidth: 180 }}>
          ...
          <button onClick={() => setShowServerConfig(!showServerConfig)} style={{ marginTop: 4, fontSize: 10, color: "#718096", background: "none", border: "none", cursor: "pointer" }}>⚙️ {showServerConfig ? "Hide" : "Server URL"}</button>
          {showServerConfig && <input type="text" value={currentServerUrl} onChange={(e) => setServerUrls((prev) => ({ ...prev, [selectedModel]: e.target.value }))} placeholder="http://localhost:8000" style={{ width: "100%", padding: "6px 8px", fontSize: 11, border: "1px solid #E2E8F0", borderRadius: 4, marginTop: 4 }} />}
        </div>
        {currentModelConfig?.requiresPrompt && (
          <div style={{ flex: 1, maxWidth: 500 }}>
            ...
            <textarea value={prompt} onChange={(e) => { setPrompt(e.target.value); setSelectedPreset(null); }} placeholder="Enter objects separated by periods: car . person . dog" style={{ width: "100%", height: 60, padding: "8px 10px", fontSize: 13, border: "1px solid #E2E8F0", borderRadius: 6, resize: "none", fontFamily: "inherit", boxSizing: "border-box" }} />
          </div>
        )}
        <div style={{ minWidth: 150 }}>
          <div style={s.sectionTitle}>Default Label</div>
          <input type="text" value={defaultLabel} onChange={(e) => setDefaultLabel(e.target.value)} placeholder="object" style={{ width: "100%", padding: "8px 10px", fontSize: 13, border: "1px solid #E2E8F0", borderRadius: 6, boxSizing: "border-box" }} />
          <div style={{ fontSize: 10, color: "#A0AEC0", marginTop: 4 }}>Used when drawing new boxes</div>
        </div>
      </div>
      ...
      <div style={s.main}>
            ...
            {images.length === 0 ? (
              ...
              images.map((img, i) => (
                <div key={img.id} onClick={() => { setCurrentImageIndex(i); setSelectedRegions([]); setZoom(1); setActiveFilter(null); }} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px", marginBottom: 4, borderRadius: 6, cursor: "pointer", background: i === currentImageIndex ? PRIMARY_LIGHT : "#FFF", border: `1px solid ${i === currentImageIndex ? PRIMARY_BORDER : "#E2E8F0"}` }}>
                  <div style={{ width: 36, height: 36, borderRadius: 4, overflow: "hidden", background: "#E2E8F0", flexShrink: 0 }}><img src={img.url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /></div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 10, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{img.name}</div>
                    <div style={{ fontSize: 9, color: "#A0AEC0" }}>{(regions[img.id] || []).length} objects</div>
                  </div>
                  <button onClick={(e) => handleDeleteImage(img.id, e)} title="Remove image" style={{ padding: "2px 6px", fontSize: 12, background: "transparent", border: "none", color: "#A0AEC0", cursor: "pointer", borderRadius: 4, lineHeight: 1 }} onMouseEnter={(e) => { e.target.style.background = "#FED7D7"; e.target.style.color = "#E53E3E"; }} onMouseLeave={(e) => { e.target.style.background = "transparent"; e.target.style.color = "#A0AEC0"; }}>×</button>
                </div>
              ))
            )}
          </div>
        ...
        <div style={s.canvas}>
          <div style={s.canvasToolbar}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <button onClick={() => currentImageIndex > 0 && setCurrentImageIndex((i) => i - 1)} style={{ padding: "4px 8px", background: "#4A5568", color: "#FFF", border: "none", borderRadius: 4, cursor: "pointer" }}>←</button>
              <span style={{ fontSize: 12, color: "#E2E8F0", maxWidth: 150, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{currentImage?.name || "No image"}</span>
              <span style={{ fontSize: 11, color: "#718096" }}>{currentImageIndex + 1}/{Math.max(1, images.length)}</span>
              <button onClick={() => currentImageIndex < images.length - 1 && setCurrentImageIndex((i) => i + 1)} style={{ padding: "4px 8px", background: "#4A5568", color: "#FFF", border: "none", borderRadius: 4, cursor: "pointer" }}>→</button>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              {Object.entries(ANNOTATION_MODES).map(([key, mode]) => (<button key={key} onClick={() => setAnnotationMode(key)} title={`${mode.label} (${mode.shortcut})`} style={s.modeButton(annotationMode === key)}><span>{mode.icon}</span><span>{mode.label}</span></button>))}
              ...
              <span style={{ fontSize: 11, color: "#E2E8F0", minWidth: 45, textAlign: "center" }}>{Math.round(zoom * 100)}%</span>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <button onClick={handleUndo} disabled={historyIndex <= 0} title="Undo (Ctrl+Z)" style={{ padding: "4px 8px", background: historyIndex > 0 ? "#4A5568" : "#2D3748", color: historyIndex > 0 ? "#FFF" : "#718096", border: "none", borderRadius: 4, cursor: historyIndex > 0 ? "pointer" : "not-allowed", fontSize: 12 }}>↩</button>
              <button onClick={handleRedo} disabled={historyIndex >= history.length - 1} title="Redo (Ctrl+Shift+Z)" style={{ padding: "4px 8px", background: historyIndex < history.length - 1 ? "#4A5568" : "#2D3748", color: historyIndex < history.length - 1 ? "#FFF" : "#718096", border: "none", borderRadius: 4, cursor: historyIndex < history.length - 1 ? "pointer" : "not-allowed", fontSize: 12 }}>↪</button>
              <span style={{ fontSize: 11, color: "#718096", marginLeft: 8 }}>{currentRegions.length} objects</span>
              {currentRegions.length > 0 && (<button onClick={handleClearCurrentDetections} title="Clear all annotations (with confirmation)" style={{ padding: "4px 8px", fontSize: 11, background: "transparent", color: "#A0AEC0", border: "1px solid #4A5568", borderRadius: 4, cursor: "pointer" }} onMouseEnter={(e) => { e.target.style.background = "#E53E3E"; e.target.style.color = "#FFF"; e.target.style.borderColor = "#E53E3E"; }} onMouseLeave={(e) => { e.target.style.background = "transparent"; e.target.style.color = "#A0AEC0"; e.target.style.borderColor = "#4A5568"; }}>🗑</button>)}
            </div>
          </div>
            ...
            {currentImage ? (
              <div ref={canvasRef} onMouseDown={handleCanvasMouseDown} onMouseMove={handleCanvasMouseMove} onMouseUp={handleCanvasMouseUp} onMouseLeave={handleCanvasMouseUp} style={{ width: CANVAS_WIDTH, height: CANVAS_HEIGHT, position: "relative", overflow: "hidden", borderRadius: 8, transform: `scale(${zoom})`, transformOrigin: "center", transition: isDrawing || isDragging || isResizing ? "none" : "transform 0.15s", cursor: getCursor(), userSelect: "none" }}>
                ...
                {currentRegions.map((region) => {
                  ...
                  return (
                    <div key={region.id} onClick={(e) => handleSelectRegion(region.id, e)} onMouseDown={(e) => { if (annotationMode === "edit" && !isResizing) handleDragStart(region.id, e); }} onDoubleClick={(e) => handleDoubleClick(region.id, e)} onMouseEnter={() => setHoveredRegion(region.id)} onMouseLeave={() => setHoveredRegion(null)} style={{ position: "absolute", left: scaled.x, top: scaled.y, width: scaled.w, height: scaled.h, border: `${isSelected ? 3 : 2}px solid ${color}`, background: isSelected ? `${color}30` : isHovered ? `${color}15` : "transparent", borderRadius: 2, cursor: annotationMode === "edit" ? "move" : annotationMode === "select" ? "pointer" : "default", boxSizing: "border-box", opacity: isDimmed ? 0.25 : 1, transition: "opacity 0.2s" }}>
                      {showLabel && !isDimmed && (<div style={{ position: "absolute", top: -22, left: -2, padding: "2px 6px", background: color, borderRadius: 3, fontSize: 10, fontWeight: 600, color: "#FFF", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 4 }}>{region.status === "manual" && <span style={{ opacity: 0.8 }}>✎</span>}{region.label}{showConfidence && <span style={{ opacity: 0.8 }}>{(region.confidence * 100).toFixed(0)}%</span>}</div>)}
        ...
        <div style={s.rightPanel}>
          ...
          <div style={{ padding: "12px", borderBottom: "1px solid #E2E8F0" }}>
            ...
            {showThresholds && currentModelConfig?.thresholds && (<div style={{ marginBottom: 8 }}>{Object.entries(currentModelConfig.thresholds).map(([k, v]) => (<div key={k} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}><span style={{ fontSize: 10, color: "#718096", minWidth: 60 }}>{v.label}</span><input type="range" min={v.min} max={v.max} step={0.05} value={thresholds[k] || v.default} onChange={(e) => { setThresholds((p) => ({ ...p, [k]: parseFloat(e.target.value) })); setSensitivityPreset(null); }} style={{ flex: 1, cursor: "pointer" }} /><span style={{ fontSize: 10, fontWeight: 600, minWidth: 30 }}>{((thresholds[k] || v.default) * 100).toFixed(0)}%</span></div>))}</div>)}
            ...
            <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#718096", cursor: "pointer", marginTop: 10 }}><input type="checkbox" checked={showConfidence} onChange={(e) => setShowConfidence(e.target.checked)} />Show confidence scores</label>
          </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {filterChips.map((chip) => (
                <FilterChip
                  ...
                  count={chip.count}
                  color={chip.color}
                  isActive={chip.label === "All" ? !activeFilter : activeFilter === chip.label}
          ...
          {/* Labeled Objects - Grouped with Accordions (NEW) */}
          <div style={{ padding: "10px 12px", borderBottom: "1px solid #E2E8F0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ flex: 1, overflow: "auto", padding: 8 }}>
            {currentRegions.length === 0 ? (
              <div style={{ textAlign: "center", padding: 24, color: "#A0AEC0", fontSize: 11 }}>
                {annotationMode === "draw" ? (<><div style={{ fontSize: 24, marginBottom: 8 }}>✎</div><div>Draw boxes to annotate</div></>) : "No objects detected"}
              </div>
            ) : (
              groupedRegions
                .filter(([label]) => !activeFilter || label === activeFilter)
                .map(([label, items]) => (
                  <CategoryAccordion
                    key={label}
                    label={label}
          ...
          <div style={{ padding: "12px", borderTop: "1px solid #E2E8F0", background: "#F7FAFC" }}>
            <div style={{ fontSize: 11, color: "#718096", marginBottom: 8 }}>{selectedRegions.length > 0 ? `${selectedRegions.length} selected` : "Select objects to review"}</div>
            <div style={{ display: "flex", gap: 6 }}>
              <button onClick={handleApprove} disabled={!selectedRegions.length} style={{ flex: 1, padding: "6px", fontSize: 11, background: selectedRegions.length ? "#38A169" : "#E2E8F0", color: selectedRegions.length ? "#FFF" : "#A0AEC0", border: "none", borderRadius: 4, cursor: selectedRegions.length ? "pointer" : "not-allowed" }}>✓ Approve</button>
              <button onClick={handleFlag} disabled={!selectedRegions.length} style={{ flex: 1, padding: "6px", fontSize: 11, background: selectedRegions.length ? "#FFF" : "#E2E8F0", color: selectedRegions.length ? "#D69E2E" : "#A0AEC0", border: "1px solid", borderColor: selectedRegions.length ? "#D69E2E" : "#E2E8F0", borderRadius: 4, cursor: selectedRegions.length ? "pointer" : "not-allowed" }}>⚠ Flag</button>
              <button onClick={handleDelete} disabled={!selectedRegions.length} style={{ flex: 1, padding: "6px", fontSize: 11, background: selectedRegions.length ? "#FFF" : "#E2E8F0", color: selectedRegions.length ? "#E53E3E" : "#A0AEC0", border: "1px solid", borderColor: selectedRegions.length ? "#E53E3E" : "#E2E8F0", borderRadius: 4, cursor: selectedRegions.length ? "pointer" : "not-allowed" }}>🗑 Delete</button>
            </div>
          </div>
      ...
      <div style={{ padding: "6px 20px", background: "#FFF", borderTop: "1px solid #E2E8F0", fontSize: 10, color: "#718096", display: "flex", justifyContent: "space-between" }}>
        <div style={{ display: "flex", gap: 16 }}><span>V Select</span><span>D Draw</span><span>E Edit</span><span>← → Navigate</span><span>+ − Zoom</span><span>Del Delete</span><span>Ctrl+Z Undo</span><span>Esc Clear Filter</span></div>
