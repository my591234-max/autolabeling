...
export default function AutoLabelProV6() {
  ...
  const handleDetectAll = async () => {
    ...
    try {
      for (const img of images) {
        processedCount++;
        showToast(
          `Processing ${processedCount}/${images.length}: ${img.name}`,
          "info",
        );
        ...
        try {
          ...
          const detections = (result.detections || []).map((det, idx) => ({
            id: Date.now() + idx + processedCount * 10000,
            label: det.label || det.class || "object",
            confidence: det.confidence || det.score || 0,
            status: "auto",
            x: det.bbox?.[0] || 0,
          ...
          console.error(`Error detecting ${img.name}:`, err);
          // Continue with other images even if one fails
        }
      }

      // Update all regions at once
      setRegions((prev) => ({ ...prev, ...allNewRegions }));
      setCurrentStep(3);
      showToast(
        `Completed! Detected ${totalDetected} objects in ${images.length} images`,
        "success",
      );
    } catch (err) {
      showToast(`Detection failed: ${err.message}`, "error");
  ...
  // Clear all detections for the current image
  const handleClearCurrentDetections = () => {
    if (!currentImage) return;
    const count = currentRegions.length;
    if (count === 0) {
      showToast("No detections to clear", "warning");
      return;
    }
    setRegions((prev) => ({ ...prev, [currentImage.id]: [] }));
    setSelectedRegions([]);
    showToast(`Cleared ${count} detections`, "info");
  };

  // Clear all detections for all images
  const handleClearAllDetections = () => {
    ...
    setRegions({});
    setSelectedRegions([]);
    showToast(`Cleared all ${totalRegionsAllImages} detections`, "info");
  };
  ...
  return (
    <div style={s.container}>
      ...
      <div style={s.controls}>
              ...
              <optgroup key={cat} label={cat}>
                {models.map((m) => (
                  <option
                    ...
                    disabled={m.disabled}
                    style={{
                      color: m.disabled ? "#A0AEC0" : "inherit",
                      fontStyle: m.disabled ? "italic" : "normal",
                    }}
                  >
                    {m.name}
                  </option>
        ...
        {/* Prompt Panel - Presets moved to the right */}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          <button
            ...
            disabled={
              isDetecting || images.length === 0 || modelStatus !== "connected"
            }
            style={{
              padding: "8px 16px",
              fontSize: 12,
          ...
          >
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {images.length > 0 && (
                <button
                  onClick={() => {
                    if (window.confirm("Remove all images and detections?")) {
                      setImages([]);
                      setRegions({});
          ...
          <div style={s.canvasToolbar}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              {currentRegions.length > 0 && (
                <button
                  onClick={handleClearCurrentDetections}
                  title="Clear all detections for this image"
                  style={{
                    padding: "4px 8px",
                    fontSize: 10,
                    background: "#E53E3E",
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              {currentRegions.length > 0 && (
                <button
                  onClick={handleClearCurrentDetections}
                  title="Clear detections for this image"
                  style={{
                    padding: "2px 6px",
                    fontSize: 10,
                    background: "#FFF",
